...
<4> A base directory from which files specified by `$extends` attribute are searched..
<5> A base directory from which "driver" files are searched.
<6> A directory to which `commandunit` writes report.
(report directory, can be changed through `--test-reportdir=` option)
<7> A directory to which `commandunit` stores processed test scripts and intermediate files. (working directory, can be changed through `--test-workdir` option)
